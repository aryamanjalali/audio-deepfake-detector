...
def download_ljspeech_subset():
    ...
    # Get first 1000 wav files for real samples
...
def download_wavefake_subset():
    ...
    # For demo, I'll create instructions since WaveFake is large
    # In a real scenario, we'd download from Zenodo
    ...
    # Download from a smaller public source or use Mozilla TTS samples
    # For now, let's use a workaround with vocoder-generated samples
...
def generate_fake_from_real(data_dir: Path):
    ...
    # Use ffmpeg to create "fake-sounding" audio with heavy processing
    import subprocess
    real_files = list(real_dir.glob("*.wav"))[:500]  # Use half for fakes
    ...
    for i, real_file in enumerate(tqdm(real_files, desc="Creating synthetic fakes")):
        try:
            # Apply extreme codec abuse to make it sound "fake"
            # This simulates what vocoder artifacts might sound like
            output_path = fake_dir / f"fake_{i:04d}.wav"
            
            # Multi-stage compression to degrade quality (simulates vocoder artifacts)
            cmd = [
                'ffmpeg', '-nostdin', '-y', '-i', str(real_file),
                '-af', 'aresample=8000,aresample=16000',  # Downsample and upsample
                '-ar', '16000',
                '-ac', '1',
                '-c:a', 'libopus',
                '-b:a', '8k',
